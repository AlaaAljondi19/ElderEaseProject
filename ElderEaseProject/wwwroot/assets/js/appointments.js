...
// دالة لجلب البيانات عند تحميل الصفحة (أضفتها لضمان ظهور المواعيد من السيرفر)
    ...
    fetch('/api/Appointments')
        ...
        .then(data => {
            data.forEach(app => {
                ...
                appendApptDOM(app.id, app.title, app.type, d.getDate(), MONTHS[d.getMonth()], formatTime(app.time));
            });
            checkEmpty();
        });
});
...
function addAppt(e) {
    ...
    // تفعيل الإرسال للسيرفر (المهمة 3 - POST)
    fetch('/api/Appointments', {
        method: 'POST',
...
function appendApptDOM(id, title, type, day, month, timeStr) {
    ...
    if (id) div.dataset.id = id; // تخزين الـ ID القادم من السيرفر
    div.innerHTML = `
...
function deleteAppt(btn) {
    ...
    // تفعيل الحذف من السيرفر (المهمة 3 - DELETE)
    if (id) {
